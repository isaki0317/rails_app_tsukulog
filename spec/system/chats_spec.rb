...
describe 'チャット機能のテスト' do
  ...
  let!(:test_user_room) { build(:user_room, end_user_id: test_user.id, room_id: test_room.id) }

  let(:test_user_2) { create(:end_user) }
  let!(:test_user_room_2) { build(:user_room, end_user_id: test_user_2.id, room_id: test_room.id) }
